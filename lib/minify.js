...
import { process_original_scopes, process_generated_ranges } from "./sourcemap-scopes.js";
        ...
        if (options.parse.spidermonkey) {
            ...
            for (var name in files) if (HOP(files, name)) {
                ...
                if (options.sourceMap && options.sourceMap.scopes) {
                  options.parse.toplevel.figure_out_scope(options.mangle);
                  options.sourceMap.originalScopes ||= [];
                  options.sourceMap.originalScopes.push(process_original_scopes(options.parse));
                }
            }
        }
    ...
    if (!HOP(options.format, "code") || options.format.code) {
        ...
        if (options.sourceMap) {
            ...
            format_options.source_map = yield* SourceMap({
                ...
                originalScopes: options.sourceMap.scopes ? options.sourceMap.originalScopes : null,
            });
        }
        ...
        if (options.beautify) {
          // format_options.max_line_len = true;
        }
        ...
        if (options.sourceMap) {
            Object.defineProperty(result, "map", {
                ...
                enumerable: true,
                get() {
                    const map = format_options.source_map.getEncoded(options.sourceMap.scopes ? process_generated_ranges(toplevel) : null);
